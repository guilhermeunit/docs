...
<Note>
  **Banco PAN — fluxo assíncrono (autorização por WhatsApp).** A consulta
  ...
  retorna `provider_unavailable` com `retryable: true`. Concluída a
  autorização (webhook `authorization_result` avisa), a próxima consulta
  devolve as ofertas precificadas com `offerId` — daí em diante o fluxo é o
  padrão (submit → status → formalize com link digital).
</Note>
...
<Note>
  **Crefaz — fluxo assíncrono (autorização por SMS).** A consulta dispara um
  ...
  ação pendente e um `submit` retorna `provider_unavailable` com
  `retryable: true` — re-tente após a autorização. Depois da Mesa aprovar, o
