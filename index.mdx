...
<Note>
  ...
  o submit usa (`offer_ref.offer_id`), igual em produção. Re-simular com
  `loan_value`/`valor_parcela`/`prazo` sempre devolve `offerId` novo, mesmo
  em sequência. Só a repetição da **mesma** consulta sem parâmetros em menos
  de 60 segundos pode vir com `offerId: null` — aguarde e re-consulte.
</Note>
