...
<ParamField body="originador" type="string">
  Identificador de originação/atribuição. **Default: a identidade do seu
  cadastro de parceiro** — na maioria dos casos não envie este campo; ele só
  existe para contratos que operam múltiplas origens sob a mesma chave.
</ParamField>
...
<ParamField body="offer_ref" type="object" required>
  ...
  em cada oferta da consulta** — ele identifica a oferta persistida e funciona
  para qualquer banco. Para **Facta** e **Unit**, a API resolve a oferta
  server-side (na Facta, a simulação é re-executada no momento do submit — o
  identificador nunca expira). Compat: os ids nativos do `providerData`
  (`id_simulacao` no C6, `proposalNumber` na Zipdin, `simulationId` na Unit)
  continuam aceitos.
</ParamField>
