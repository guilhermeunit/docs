...
<ParamField body="telefone" type="string" required>
  ...
  remove automaticamente. Qualquer outro formato retorna `422 invalid_input`.
</ParamField>

<ParamField body="email" type="string" required>
  E-mail do titular, em formato válido. Vazio ou inválido retorna
  `422 invalid_input` — o HubCrédito não processa consultas sem e-mail.
</ParamField>
...
<ParamField body="providers" type="string[]">
  ...
  Desde 13/08: se **nenhum** nome pedido estiver no seu allowlist (ex.: typo
  ...
  vez de responder `200` com `providerResults` vazio.
</ParamField>

<ParamField body="sexo" type="integer" default="0">
  Sexo do titular na **consulta**. Hoje só a **HubCrédito** usa este campo na
  simulação: `0` = masculino, qualquer outro valor (use `1`) = feminino — não
  existe "não informado", o default `0` vai como masculino. Os demais bancos
  ignoram; se a HubCrédito não está no seu allowlist, pode omitir. (Na
  formalização o sexo vai em `titular.sexo` como `"M"`/`"F"`.)
</ParamField>
...
<ParamField body="loan_value" type="number">
  **Modo "por valor final"**: simula o valor de empréstimo desejado. Envie
  junto com `prazo`; se `prazo` for omitido, a Unit assume **48 parcelas**
  (o mesmo default do nosso formulário) e ajusta automaticamente quando o
  prazo não está disponível para o cliente. Opcional.
</ParamField>

<ParamField body="prazo" type="integer">
  Número de parcelas desejado. Acompanha `loan_value` (por valor) ou
  `valor_parcela` (por parcela). Opcional — default 48 no modo por valor.
</ParamField>
...
<ParamField body="valor_parcela" type="number">
  **Modo "por parcela"**: simula pela parcela mensal desejada — o valor é
  ...
  parcela). Envie junto com `prazo`. Opcional. Quando `loan_value` e
  `valor_parcela` vêm juntos, o valor tem prioridade.
</ParamField>
...
<ResponseField name="proposals" type="array">
  ...
  Cada oferta também traz **`adjusted`** (bool) e **`adjustments`** (array de
  ...
  melhor possível e o ajuste sinalizado — ex. `"prazo solicitado (36x)
  indisponível — melhor prazo: 18x"` ou `"valor solicitado acima do máximo
  disponível"`. Use-os para **não** apresentar a simulação como se fosse
  exatamente a pedida.
</ResponseField>
...
<ResponseField name="employers" type="array">
  Empregadores únicos (por CNPJ) encontrados para o titular: `{cnpj, name,
  matricula, provider, margin, admission_date, salary}`. **`margin`** é a
  margem consignável disponível, apurada **em tempo real na consulta**,
  normalizada em número (`float`) a partir do que o banco do vínculo informa
  (Unit e Facta trazem hoje). **`admission_date`** (`YYYY-MM-DD`; Unit e
  Facta) e **`salary`** (`float`; só a Unit) servem para pré-preencher o bloco
  `vinculo` da formalização — a Facta exige `data_admissao` e `salario` —
  sem perguntar ao cliente. Qualquer um vem `null` quando o banco do vínculo
  não informa. Não confundir `margin` com o `consigned_credit_balance` do
  lookup de leilão, que é o retrato no momento em que o convite foi gerado.
</ResponseField>
...
```text
GET /partners/v1/auction-proposals/{proposal_key}
```

Sem efeitos colaterais: a consulta não cria proposta, não dispara SMS e não
consome limites do cliente. Chame quantas vezes precisar.

```bash
curl "https://api.somosunit.com.br/partners/v1/auction-proposals/{proposal_key}" \
...
```json
{
  "proposal_key": "fba60e8e-...",
  "name": "NOME DO CLIENTE",
  "document_number": "12345678901",
  "birth_date": "1994-03-10",
  "admission_date": "2023-05-15",
  "employer_document_number": "04021583000198",
  "issuer_registration_number": "000001234",
  "eligible": true,
  "consigned_credit_balance": 910.0,
  "number_of_installments": 12,
  "political_exposition": "not_exposed",
  "status": "ongoing",
  "proposal": {
    "auction_proposal_key": "6b931708-...",
    "status": "bid",
    "proposal_data": {
      "disbursed_issue_amount": 2500.0,
      "monthly_interest_rate": 0.0496,
      "number_of_installments": 12,
      "simulation": {
        "installment_face_value": 313.74,
        "total_iof": 67.3,
        "monthly_cet": 0.0533
      }
    }
...
- **`disbursed_issue_amount` na raiz** = valor **pedido pelo trabalhador** no
  leilão da CTPS. **`proposal.proposal_data.disbursed_issue_amount`** = valor
  da **proposta enviada** (o lance real). Apresente ao cliente sempre o de
  `proposal_data`, usando `proposal_data.simulation` para parcela/CET; o da
  raiz serve para contexto ("você pediu X, conseguimos Y").
- **`consigned_credit_balance`** = margem disponível **no momento em que o
  convite do leilão foi gerado** — pode estar defasada. A margem em tempo
  real é a `employers[].margin` da consulta. Para régua de elegibilidade, use
  este campo como pré-filtro barato e a margem da consulta como critério
  final.
- **Campos de repasse** (`webhook_type`, `alerts`, `rank_position`,
  `event_datetime`, `inclusion_limit_datetime`, `external_proposal_url` e as
  chaves internas `*_key`): vêm da instituição como estão, podem vir `null`
  (o `rank_position` inclusive em produção) e podem mudar sem aviso — não
  construa lógica obrigatória sobre eles. O que tem prazo é o **lance**
  (`expiration_datetime`); a chave em si continua resolvendo depois disso.

| Status | Quando |
|---|---|
| `200` | Chave resolvida — payload acima |
| `401` | `X-Partner-Api-Key` ausente ou inválida |
| `403` | Seu contrato não tem a Unit no allowlist (a resposta contém dados pessoais originados na Unit) |
| `404` | `proposal_key` desconhecida ou expirada |
| `502` | Falha temporária no banco parceiro — tente novamente |

<Note>
  A `proposal_key` é uma **capability**: opaca, não-enumerável, e chega até
...
<Note>
  **No sandbox**, use as chaves de teste `sbx-leilao-ok` (retorna `200` com um
  ...
  `sbx-leilao-expirada` (retorna `404`, para testar seu tratamento de erro).
</Note>
...
<Note>
  **C6 — a biometria vem antes da oferta.** Diferente dos outros bancos, o C6 só
  ...
  `action` (`type: "biometria"`). Quando o cliente conclui:

  1. a Unit envia o webhook [`authorization_result`](#webhooks) com as ofertas do C6; e
  2. as ofertas também passam a aparecer numa nova consulta do mesmo CPF (fallback caso você não use webhook).

  Só então siga para a formalização. O `GET /partners/v1/proposals/{id}` **não**
  se aplica nessa espera — ainda não existe proposta (o submit vem depois da oferta).
</Note>

## Autorização com colheita de evidência (fluxo WhatsApp)

Atendimentos **100% WhatsApp** não têm como colher a evidência de autorização
que Zipdin, Unit e Facta exigem (IP do titular, geolocalização, dispositivo).
Para esses casos, a API gera um **link de autorização**: o cliente abre uma
página compacta, lê o termo LGPD e autoriza com um toque — a evidência é
colhida ali (IP real capturado no servidor, geolocalização, dispositivo,
versão do termo) e fica registrada na Unit como fonte da verdade.
...
<Steps>
  <Step title="Crie a sessão">
    ...
    pendente viva do mesmo CPF devolve o **mesmo** link.
  </Step>
  <Step title="Envie o link na conversa">
    O cliente abre a `url`, vê o termo com o nome dele e CPF mascarado, e
    autoriza com um toque. Localização negada não bloqueia (fica registrada
    como não-colhida).
  </Step>
  <Step title="Receba o sinal">
    Webhook `authorization.completed` no seu `webhook_url` (mesma assinatura
    HMAC do `authorization_result`) — ou poll em
    `GET /partners/v1/authorizations/{authorization_id}`, que devolve o
    status e, quando autorizada, a `evidence` completa:

    | Campo | Significado |
    |---|---|
    | `client_ip` | **IP do cliente final que autorizou** — colhido pelo servidor no momento do aceite (fonte primária de evidência) |
    | `ip_servidor` | Mesmo valor de `client_ip` (nome legado; mantido por compatibilidade) |
    | `ip_frontend` | IP que o browser do cliente auto-reportou (secundário; pode divergir com VPN/proxy ou vir vazio) |
    | `user_agent` | User-agent do browser no aceite |
    | `geo` | `latitude`, `longitude`, `cidade`, `uf` (quando o cliente permitiu localização) |
    | `device` | `operationalSystem`, `deviceModel`, `deviceName`, `deviceType` |
    | `termo_versao` | Versão exata do termo LGPD exibido e aceito |
    | `signature_date` | Timestamp do aceite |
  </Step>
  <Step title="Consulte com o authorization_id">
    Nas rotas de consulta, envie `authorization_id` **no lugar de**
    `client_ip`: a evidência colhida é injetada no servidor — sua integração
    nunca transita IP/geo do cliente.
  </Step>
</Steps>

```bash
curl -X POST "https://api.somosunit.com.br/partners/v1/authorizations" \
  -H "X-Partner-Api-Key: au_xxxxxxxx" -H "Content-Type: application/json" \
  -d '{"cpf": "12345678901", "nome": "NOME DO CLIENTE", "telefone": "27999990000"}'
```

| Situação | Resposta |
|---|---|
| Consulta sem `client_ip` **e** sem `authorization_id` | `422` |
| `authorization_id` de outro parceiro | `404` |
| Sessão ainda pendente ou expirada | `409` |
| `authorization_id` de outro CPF | `422` (evidência é pessoal) |
| Link aberto após expirar | página instrui a pedir novo link; `POST` gera outra sessão |

<Note>
  **No sandbox** o fluxo inteiro é testável por API: as rotas `public/*` são
  ...
  **"Autorização — colheita de evidência"** com os 5 passos prontos.
</Note>
...
```json
{
  ...
  "contato": { "telefone": "11999998888", "email": "fulano@example.com" },
  "endereco": {
    "cep": "01310100",
...
<ParamField body="offer_ref" type="object" required>
  A oferta escolhida — `{ provider, offer_id }`. **Envie o `offerId` que veio
  ...
  os identificadores nativos do `providerData` continuam aceitos.
</ParamField>
...
<ParamField body="titular" type="object" required>
  ...
  `nacionalidade` é **case-insensitive e tolerante**: `br`, `BR`,
  `Brasileiro`, `Brasileira` ou vazio são todos tratados como brasileiro em
  todos os bancos; só texto contendo "estrangeir" é tratado como estrangeiro.
...
<ParamField body="recebimento" type="object" required>
  ...
  consulte a lista aceita em [`GET /partners/v1/banks`](#lista-de-bancos).

  - `tipo_conta`: `corrente`, `poupanca` ou `salario` (a API traduz para o
    enum de cada banco). Máscaras em agência/conta/CEP são removidas
    automaticamente.
  - `pix.tipo_chave`: `cpf`, `telefone`, `email` ou `aleatoria`. Chave de
    CPF/celular é normalizada para o formato do banco. **Zipdin não aceita
    PIX por telefone** — nesse caso o desembolso vai para a chave PIX-CPF do
    titular.
</ParamField>
...
| Bloco | Zipdin | Unit | C6 | Facta | Crefaz | Banco PAN |
|---|:--:|:--:|:--:|:--:|:--:|:--:|
| `endereco` | obrigatório | obrigatório | obrigatório | obrigatório | obrigatório | opcional |
| `recebimento` | conta ou PIX | conta ou PIX¹ | só conta | conta ou PIX | só conta | conta (opcional) |
| `documento` (RG) | obrigatório | obrigatório | — | obrigatório | obrigatório | opcional |
| `titular.estado_civil` | obrigatório | obrigatório | — | obrigatório | — | obrigatório |
| `titular.nome_mae` | obrigatório | obrigatório | — | obrigatório | obrigatório | — |
| `titular.nome_pai` / naturalidade | opcional | — | — | obrigatório | naturalidade obrigatória | — |
| `vinculo` | obrigatório² | opcional | — | obrigatório (CNPJ + admissão + renda) | opcional | obrigatório (matrícula) |
| `titular.sexo` | obrigatório | opcional | — | obrigatório | obrigatório | obrigatório |
| `assinatura.ip` | — | obrigatório | — | — | — | — |
| `assinatura.tipo_envio` | — | — | — | obrigatório (envio do link) | — | — |
| `referencias` (2+) | — | — | — | — | obrigatório³ | — |
...
<Note>
  **Banco PAN — fluxo assíncrono (autorização por WhatsApp).** A consulta
  ...
  precisa enviar a mensagem para autorizar. Enquanto isso, um `submit`
...
<Note>
  **Crefaz — fluxo assíncrono (autorização por SMS).** A consulta dispara um
  SMS DataPrev pro cliente; enquanto ele não autorizar, a consulta volta com
  ação pendente e um `submit` retorna **`422 invalid_input`** apontando
...
| `awaiting_signature` | Link de assinatura disponível, aguardando o cliente. Aparece na resposta do submit/formalize **e no `GET` de status** enquanto o contrato não é assinado. |
...
| `provider_rejected` | 422 | O banco recusou a proposta ou os dados enviados (validação de campo, prazo, margem, assinatura). **Não re-tente com os mesmos dados** — corrija e reenvie. |
...
```json
{ "banks": [ { "code": "001", "name": "Banco do Brasil (BB)" },
             { "code": "104", "name": "Caixa Econômica Federal (CEF)" },
             { "code": "260", "name": "Nubank (Nu Pagamentos)" } ] }
```

É uma lista curada de referência para o seu formulário — a aceitação final
de cada instituição é validada pelo banco de crédito no submit/formalização.
...
| `12345678909` | C6 — auto-flip da biometria: a 1ª consulta retorna `requires_action` + link; ~15s depois a consulta passa a retornar as ofertas e dispara o webhook `authorization_result` (configure um `webhook_url` no cadastro sandbox para recebê-lo) |
| `50077799976` | Banco PAN — autorização WhatsApp pendente: `requiresSignature: true` com link `wa.me`; um submit sem `offerId` volta `422 invalid_input` |
| `40066688809` | Banco PAN — consulta ok pós-autorização (oferta com `offerId`, pré-requisito da formalização) |
| `30055577741` | Crefaz — SMS DataPrev pendente: `requiresSignature: true` com `signatureUrl: null` (autorização fora de link) |
| `20044466684` | Crefaz — consulta ok (ofertas com `offerId`, pré-requisito da formalização) |

<Note>
  **Re-simulação no sandbox** (14/08): `prazo`, `loan_value` e
  ...
  não para validar precificação.
</Note>

Além dos CPFs, o fluxo de leilão tem **chaves de teste** próprias
(`GET /auction-proposals/{key}`): `sbx-leilao-ok` → `200` e
`sbx-leilao-expirada` → `404` — ver [Fluxo de leilão](#fluxo-de-leilão-lookup-por-proposal-key).


<Note>
  Use `POST /partners/v1/consultas` (agregada) antes do submit — não a rota
  ...
  `simulationId` do `providerData` muda a cada consulta na Unit.
</Note>

### Collection do Postman

Baixe a collection com todos os endpoints (consulta e formalização) e os dois
environments prontos (produção e sandbox):

- [Collection](/postman/autounit-partners.postman_collection.json)
- [Environment — produção](/postman/autounit-partners.postman_environment.json)
- [Environment — sandbox](/postman/autounit-partners-sandbox.postman_environment.json)

Importe os 3 arquivos no Postman, selecione o environment desejado e preencha
`apiKey` (produção) — no sandbox a chave já vem preenchida.
...
<Note>
  ...
  O evento `authorization_result` do **C6** (pós-biometria) já está **no ar**:
  quando o cliente conclui a prova de vida, a Unit envia o webhook com as ofertas
  do C6. O Banco PAN segue a mesma forma quando entrar no fluxo automatizado.
</Note>
